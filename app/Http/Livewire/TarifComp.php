...
class TarifComp extends Component
{
    ...
    public function render()
    {
        return view('livewire.tarifs.index', [
            "tarifs" =>Tarification::with(["dureeLocation"])->whereArticleId($this->article->id)->get(),
    ...
    public function saveTarif(){
                ...
                $uniqueRule = function() use($newTarif,$articleId){
                    ...
                    return (isset($newTarif["edit"]))?
                     Rule::unique((new Tarification())->getTable(),"duree_location_id")->ignore($newTarif["id"],"id")->where(function($query) use ($articleId){
                        return $query->where("article_id", $articleId);
                    })
                    :

                    Rule::unique((new Tarification())->getTable(),"duree_location_id")->where(function($query) use ($articleId){
                        return $query->where("article_id", $articleId);
                    });
                };
                ...
                $this->validate([
                    "newTarif.duree_location_id"=>["required", $uniqueRule()],
                    "newTarif.prix"=>"required|numeric"
                ],
                ["newTarif.duree_location_id.unique"=>"Il existe déjà un tarif pour cette durée de location...."]
            );
